...
trait HasRoleAndPermission
{
     ...
     */
    public function roles($teamId = null)
    {   
    ...
    {
        return $this->isDeveloper() ? true : $this->getRoles()->contains(function ($value, $key) use ($role) {
    ...
    public function may($permission, $all = false)
    {
        ...
        return $this->{$this->getMethodName('may', $all)}($this->getArrayFrom($permission));
    }
     ...
     * @return bool
     */
    protected function mayOne(array $permissions)
    {
     ...
     * @return bool
     */
    protected function mayAll(array $permissions)
    {
        foreach ($permissions as $permission) {
            if (!$this->hasPermission($permission)) {
                return false;
    ...
    {
        return $this->isDeveloper() ? true : $this->getPermissions()->contains(function ($value, $key) use ($permission) {
